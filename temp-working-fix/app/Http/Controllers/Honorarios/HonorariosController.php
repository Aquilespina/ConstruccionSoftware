...
class HonorariosController extends Controller
{
    ...
    public function generarPDF(string $id)
    {
        try {
            ...
            // Obtener los detalles del honorario (solo los existentes)
            $detalles = DB::table('detalle_honorario')
                ->where('id_honorario', $id)
                ->get();

            if ($detalles->isEmpty()) {
                return response()->json(['error' => 'No se encontraron detalles para este honorario'], 404);
            }
            ...
            $saldoPendiente = $subtotalActual - $totalPagadoReal;
            $porcentajePagado = $subtotalActual > 0 ? ($totalPagadoReal / $subtotalActual * 100) : 0;

            // Usar la función para calcular estados de pago correctamente
            ...
            // Generar PDF
            $pdf = PDF::loadView('honorarios.pdf', $data);
    ...
    /**
     * Calcular el estado de pago de cada detalle basado en los pagos realizados
     * Distribuye los pagos en orden cronológico a los conceptos más antiguos
     */
    private function calcularEstadoPagosDetalles($detalles, $pagos)
    {
        ...
        // Ordenar detalles por ID (más antiguos primero)
        $detallesOrdenados = $detalles->sortBy('id_detalle');

        // Calcular total de pagos
        $montoRestante = $pagos->sum('monto');

        // Procesar cada detalle
        $detallesConEstado = $detallesOrdenados->map(function($detalle) use (&$montoRestante, $pagos) {
            $montoConcepto = (float) $detalle->importe;
            
            // Inicializar propiedades - CORREGIDO: usar nombres consistentes
            $detalle->monto_pagado_detalle = 0;
            $detalle->fecha_pago = null;
            $detalle->tipo_pago = null;
            
            if ($montoRestante > 0) {
                if ($montoRestante >= $montoConcepto) {
                    // Pago completo del concepto
                    $detalle->monto_pagado_detalle = $montoConcepto;
                    $montoRestante -= $montoConcepto;
                } else {
                    // Pago parcial del concepto
                    $detalle->monto_pagado_detalle = $montoRestante;
                    $montoRestante = 0;
                }
                
                // Asignar información del primer pago
                $primerPago = $pagos->first();
                if ($primerPago) {
                    $detalle->fecha_pago = $primerPago->fecha_pago;
                    $detalle->tipo_pago = $primerPago->tipo_pago ?? 'Efectivo';
                }
            }

            // Calcular estado de pago
            if ($detalle->monto_pagado_detalle >= $montoConcepto) {
                $detalle->estado_pago = 'Pagado';
            } elseif ($detalle->monto_pagado_detalle > 0) {
            ...
            $detalle->saldo_concepto = $montoConcepto - $detalle->monto_pagado_detalle;
